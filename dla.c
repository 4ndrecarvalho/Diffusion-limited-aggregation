...
int main(){
	int j, kia = 0, i = 1, nParticles = 1000, stepSize = 1, nSteps = 5000, skipFrames = 1;
	int xLim = 120, yLim = 80;
	...
	while (i < nParticles) {
		...
		double r = sqrt(((rand() % 10001) / 10000.0) * (((xLim+yLim)/2/2)*((xLim+yLim)/2/2) - ((xLim+yLim)/4/2)*((xLim+yLim)/4/2)) + ((xLim+yLim)/4/2)*(((xLim+yLim)/4/2)));
